...
public class BlacklistCommand extends BaseCMD {
    ...
    public void onCommand(CommandArgs cmd) {
        ...
        if (args.length == 0){
            commandSender.sendMessage(CC.translate("&e/"+ cmd.getLabel() + " <player> [duration] [reason] [-s]"));
            return;
        }
        ...
        if (!parameter.validate(commandSender, data, PunishmentType.BLACKLIST)) return;
